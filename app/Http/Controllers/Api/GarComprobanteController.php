...
class GarComprobanteController extends Controller
{
    //
     //
     ...
     { 
        $lista=Gar_comprobante::with('proveedor','usuario')->orderBy('id', 'DESC')->get();
     ...
     {          
        ...
        if($request->comprobante_id!="0"){
            $comprobante=Gar_comprobante::find($request->comprobante_id);            
            ...
            $comprobante=new Gar_comprobante;   
            $comprobante->siaf=$request->siaf;
            $comprobante->documento_tipo=$request->documento_tipo;
            $comprobante->nro_doc=$request->nro_doc;
            $comprobante->importe=$request->importe;
     ...
     public function buscar(Request $request){
         //return $request;
         return Gar_comprobante::with('proveedor')->find($request->id);
     }
